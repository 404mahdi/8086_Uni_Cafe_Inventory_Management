...
PriceArray      DW 20, 10, 60, 15, 50

GRAND_TOTAL     DW 0

; DOS buffered input: maxlen, len, data...
PW_BUF          DB 8,0,8 DUP (?)
...
PrintDashboard PROC
    ; Traverse word arrays with SI (0,2,4,...) and show ID with DI
    PUSH AX
    PUSH BX
    PUSH CX
    PUSH DX
...
LowStockAudit PROC
    ; Walk stock array in word steps and compare to MIN_STOCK
    PUSH AX
    PUSH BX
    PUSH CX
    PUSH DX
...
TotalValuation PROC
    ; Sum Price * Stock across 5 items (word arrays)
    PUSH AX
    PUSH BX
    PUSH CX
    PUSH DX
...
ReadNumber PROC
    ; Parse decimal string: AX = AX*10 + digit
    PUSH BX
    PUSH CX
    PUSH DX
    PUSH SI
...
PrintNum PROC
    ; Print AX in decimal using repeated divide by 10
    PUSH AX
    PUSH BX
    PUSH CX
    PUSH DX
